...
# Study Archive UX/UI Skill — Stage 3 Draft

> **STATUS: DRAFT — NOT FINAL.** Stage 1 (UX/UI Post-Project Analysis) is complete. Stage 2 (UX/UI
> Skill Design) is complete. Stage 3 (this file's creation) is complete only after this session's own
> self-check passes. The mandatory, separate **Independent UX/UI Skill Review** has **not** occurred.
> This file MUST NOT be treated as final, fully validated, or independently reviewed until that
> separate review passes and updates this notice. Provenance: §14.
...
**Priority ordering when layers conflict** (unchanged from, and never re-derived from, Engineering
`SKILL.md` §0 — read it there): data integrity → security/authorization/ownership →
architecture/behavioral contracts → functional behavior → accessibility/usability →
visual/aesthetic preference. A UX/UI improvement never buys a lower layer's cost with a higher
layer's coin.
...
this Skill's boundary machinery (§4) requires reading it — Route 3 (§2), a failed tier-B contract
check (§4), or any STOP (§11) — actually issue a `Read` of that path in the current session. Never
assume its contents from memory of an earlier session; treat a prior session's recollection of
Engineering methodology as stale until re-read.
...
4. **`skills/study_archive/engineering/SKILL.md`** — only when the boundary machinery (§4) requires
   it (Route 3, failed tier-B check, STOP). Not read by default for ordinary Route 1/2 work.
...
Anything landing in tier C or D (§4). This route's only job is to **detect and hand off** — it does
not process the change.

- Investigation stops at classification. STOP (§11): record the finding, `Read`
  `skills/study_archive/engineering/SKILL.md`, route via its own §2/§3. No UX/UI implementation
  occurs in this route.

*Example: Case F, "translate all remaining error messages" — a many-conditional retrofit (§9.B),
routes here. Case G, redesigning Login/Register — reopens a documented Owner Decision (§13), routes
here.*
...
- Heading semantics — single `<h1>` per page is the existing pattern; preserve it. Image/decoration
  semantics — `alt` on meaningful images; `aria-hidden`/`role="img"` suppression on purely decorative
  SVGs.
...
fix. Two dark-mode activation blocks require byte-sync verification per §9.C:
...
**Stage 3** (2026-08-19, this session): first creation of this file, implementing UX-01–UX-16
faithfully against the Stage-2 specification. **Draft status — the mandatory, separate Independent
UX/UI Skill Review has not yet occurred.** This notice, and the status banner at the top of this
file, must not be altered to claim "final" or "independently reviewed" until that review actually
runs and passes.
